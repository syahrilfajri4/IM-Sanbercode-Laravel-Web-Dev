...
class m_inventory extends CI_Model
{
    ...
    public function getAll()
    {
        ...
        // Mengambil data dari form (POST request)
        $cat = $this->input->post('kategori'); // Kategori
        $status = $this->input->post('stt'); // Status
        $periode = $this->input->post('periode'); // Periode
        $tahun = $this->input->post('tahun'); // Tahun
    
        // Menetapkan nilai default untuk status jika tidak ada data yang diterima dari form
        ...
        if(!empty($cat))
        {
            if($cat == 'fg')
            {
                return $this->db->query("SELECT * FROM `plan1` WHERE `Category` IN ('Barang Jadi Rehab','Barang Jadi Furniture','Finish Goods') AND `STATUS` = '$status' AND Periode = '$periode' AND tahun = '$tahun'")->result();
            } 
            else if($cat == 'comp')
            {
                return $this->db->query("SELECT * FROM `plan1` WHERE `Category` IN ('Komponen Rehab','Komponen Furniture','Component') AND `STATUS` = '$status' AND Periode = '$periode' AND tahun = '$tahun'")->result();
            }
            else if($cat == 'cst' || $cat == 'rwk')
            {
                return $this->db->query("SELECT * FROM `plan1` WHERE `STATUS` = '$status' AND Periode = '$periode' AND tahun = '$tahun'")->result();
            }
        }
        else
        {
            // Query default jika tidak ada kategori yang dipilih
            return $this->db->query("SELECT * FROM `plan1` WHERE `Category` IN ('Barang Jadi Rehab','Barang Jadi Furniture','Finish Goods') AND `STATUS` = '$status' AND Periode = '$period' AND tahun = '$year'")->result();
        }
    }    
    ...
    public function getBarang() {
        ...
        if (empty($cat) || $cat == 'fg') {
            // Query untuk mengambil barang dari tabel tampil_bom jika kategori kosong atau 'fg'
            return $this->db->query("
                SELECT ID, IF(Series != '', Series, Item) as Series, `Item Name` as ItemName 
                FROM `tampil_bom` 
                WHERE `Category` IN ('Barang Jadi Rehab', 'Barang Jadi Furniture', 'Finish Goods') 
                ORDER BY `ID` ASC")->result();
        } else if ($cat == 'comp') {
            // Query untuk mengambil komponen dari tabel inventory jika kategori adalah 'comp'
            return $this->db->query("
                SELECT ID, IF(Series != '', Series, Item) as Series, ItemName 
                FROM `inventory` 
                WHERE `Category` IN ('Komponen Rehab', 'Komponen Furniture', 'Component') 
                ORDER BY `ID` ASC")->result();
        } else if ($cat == 'cst' || $cat == 'rwk') {
            // Query untuk mengambil barang dari tabel produksi_inventory jika kategori adalah 'cst' atau 'rwk'
            return $this->db->query("
                SELECT ID, `Item Name` as Series, `Item Name` as ItemName 
    ...
    public function JIPinput()
    {
        ...
        $kat = $this->db->query("SELECT Category FROM inventory WHERE ID = '$id_barang'")->row();

        // Menentukan series berdasarkan kategori
        if ($kat->Category == 'Komponen Rehab' || $kat->Category == 'Komponen Furniture' || $kat->Category == 'Component') {
            $show_series = 'KOMP';
        } else {
            $series = $this->db->query("SELECT Series FROM inventory WHERE ID = '$id_barang'")->row();
            $show_series = $series->Series;
        }
        ...
        // $this->db->query("
        //     INSERT INTO `erp_master_plan_sche` (`Product`, `Plan Qty`, `Ref No`, `Periode`)
        //     VALUES ('$id_barang', '$jml', '$no_ref', '$periode')
        // ");
        // Memasukkan data ke dalam tabel dengan status 1 (tidak aktif)
        $this->db->query("
        INSERT INTO `erp_master_plan_sche` (`Product`, `Plan Qty`, `Ref No`, `Periode`, `Status`)
        VALUES ('$id_barang', '$jml', '$no_ref', '$periode', 1)
        ");
    ...
    {
        $kanban = $this->input->get('id_mps');  // Gunakan kelas input CI untuk keamanan yang lebih baik

        // Mulai transaksi
        $this->db->trans_begin();

        // Ambil informasi kanban
        $kanbaninfo = $this->db->query("SELECT Product, `Plan Qty` AS qty FROM erp_master_plan_sche WHERE ID = ?", array($kanban))->row();
        if (!$kanbaninfo) {
        ...
        $bom = $kanbaninfo->Product;
        $qty = $kanbaninfo->qty;

        // Ambil informasi produk master
        $mp_id = $this->db->query("SELECT * FROM ppic_master_produk WHERE `Kode Produk` = ?", array($bom))->row();
        if (!$mp_id) {
            // Rollback transaksi jika produk master tidak ditemukan
            $this->db->trans_rollback();
            return;
        }
        ...
        foreach ($parts as $part) {
            ...
            // Masukkan rencana alokasi
            $this->db->query("INSERT INTO alokasi_plan_proto (id_part, kanban, kebutuhan, id_master_produk) VALUES (?, ?, ?, ?)", array($part->kd_part, $kanban, $total, $bom));
        }

        // Perbarui status
        $this->db->query("UPDATE erp_master_plan_sche SET `Status` = 2 WHERE ID = ?", array($kanban));
