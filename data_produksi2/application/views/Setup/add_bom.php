...
<body>
    ...
    <h5>Data BOM :</h5>
        ...
        <div class="row">
            <h5>Tambah Item BOM</h5>
        </div>
        <form action="<?= site_url('C_Bom/add_edit/'.$series->ID_BOM) ?>" method="post">
        <div class="row">
            <div class="col-sm-2 mb-2">
                <label class="form-label" for="kode_barang">Part ID:</label>
            ...
            <div class="row">
                <div class="col-sm-4 mb-2">
                    ...
                    <a class="btn btn-secondary" href="<?= site_url('C_Bom/list_BOM1/'.$series->ID_BOM) ?>">Batal</a>
                    <a class="btn btn-warning" data-bs-toggle="collapse" href="#collapseCari" role="button" aria-expanded="false" aria-controls="collapseCari">
                        Cari Barang
                    </a>
                </div>             
            </div>
        ...
        <div class="<?= $collapse ?>" id="collapseCari">
            <div class="card card-body">
                <form action="<?= site_url('C_Bom/Cari/'.$series->ID_BOM) ?>" method="post">
                    <div class="row">
                        <div class="col-sm-8 mb-2">
                ...
                <select class="form-select" size="10" multiple aria-label="multiple select example">
                    <?php foreach($inventory as $inventory) : ?>
                        <?php 
                            if($inventory->id_fina == '0') {
                                $Item = $inventory->Item ;
                            } else {
                                $Item = $inventory->id_fina ; 
                            }
                        ?>
                        <option><?= $inventory->ID.' - ('.$Item.') - '.$inventory->ItemName ?></option>
                    <?php endforeach; ?>
                </select>
                </div>
            </div>
